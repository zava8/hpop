...
import org.kibord.latinkb.Keyboard.Key;
...
import static org.kibord.latinkb.aski_hksu.aski_dijit_kya;
...
//    private final float KEY_LABEL_VERTICAL_ADJUSTMENT_FACTOR = 0.55f; // orijnql
    ...
    static class PointerQueue {
        ...
        public void releaseAllPointersOlderThan(org.kibord.latinkb.PointerTracker tracker, long eventTime) {
            LinkedList<org.kibord.latinkb.PointerTracker> queue = mQueue;
            int oldestPos = 0;
            for (org.kibord.latinkb.PointerTracker t = queue.get(oldestPos); t != tracker; t = queue.get(oldestPos)) {
    ...
    public LatinKeyboardBaseView(Context context, AttributeSet attrs, int defStyle) { super(context, attrs, defStyle);
        fontsoverride.setDefaultFont(context,"DEFAULT",R.font.latin54);
//        fontsoverride.setDefaultFont(context,"DEFAULT_BOLD",R.font.lato5r);
        if (!isInEditMode()) Log.i(TAG, "kreating niyu latinkbibordbesviyu " + this);
    ...
    public void setKeyboard(Keyboard keyboard) {
        ...
        mLabelScale = org.kibord.latinkb.LatinIME.sKeyboardSettings.labelScalePref;
        requestLayout();
        mKeyboardChanged = true;
        invalidateAllKeys();
        computeProximityThreshold(keyboard);
        mMiniKeyboardCacheMain.clear(); mMiniKeyboardCacheShift.clear(); mMiniKeyboardCacheCaps.clear();
        setRenderModeIfPossible(org.kibord.latinkb.LatinIME.sKeyboardSettings.renderMode);
        mIgnoreMove = true;
    }    
...
//                        lebl_size = (int)(mKeyTextSize * 1); // mlyalm
                    ...
                    case "m":
                        lebl_klr = 0xffffffff;
                        lebl_size = (int)(mKeyTextSize * 1.2f); // telgu mlyalm latinkb
//                        lebl_size = (int)(mKeyTextSize * 1.8f); //latin
                        lebl_baseline = top_y + labelHeight * KEY_LABEL_VERTICAL_ADJUSTMENT_FACTOR * 1.8f;
                    break;
                    case "y":
                        lebl_klr = 0xffffffff;
                        lebl_size = (int)(mKeyTextSize * 1.0f); // latinkb
//                        lebl_size = (int)(mKeyTextSize * 1.8f);
                        lebl_baseline = top_y + labelHeight * KEY_LABEL_VERTICAL_ADJUSTMENT_FACTOR * 1.8f;
                    break;
